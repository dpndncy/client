...
export class CourseService {
  ...
  public latestCourses(count: number): Observable<Course[]> {
    return this.http.get("/course/latest?count=${count}").map(response => response.json());
  }

  public searchCoursesByTag(tag: string, page: number, size: number): Observable<Course[]> {
    return this.http.get("/course/search/byTag?tag=${tag}&page=${page}&size=${size}").map(response => response.json());
  }

  public searchCoursesByCategory(category: string, page: number, size: number): Observable<Course[]> {
    return this.http.get("/course/search/byCategory?category=${category}&page=${page}&size=${size}").map(response => response.json());
  }

  public searchCoursesByName(name: string, page: number, size: number): Observable<Course[]> {
    return this.http.get("/course/search/byName?name=${name}&page=${page}&size=${size}").map(response => response.json());
  }

  public searchCourses(q: string, page: number, size: number): Observable<Course[]> {
    return this.http.get("/course/search?q=${q}&page=${page}&size=${size}").map(response => response.json());
  }

  public getCourseInfo(author: string, name: string): Observable<CourseInfo> {
    return this.http.get("/course/info?author=${author}&name=${name}").map(response => response.json());
  }

  public getModuleInfo(author: string, course: string, name: string): Observable<ModuleInfo> {
    return this.http.get("/module/info?author=${author}&name=${name}&course=${course}").map(response => response.json());
  }

  public getActivityInfo(author: string, course: string, module: string, name: string): Observable<ActivityInfo> {
    return this.http.get("/activity/info?author=${author}&name=${name}&course=${course}&module=${module}").map(response => response.json());
  }

  public searchTags(q: string, page: number, size: number): Observable<Tag[]> {
    return this.http.get("/tags/search?q=${q}&page=${page}&size=${size}").map(response => response.json());
  }

  public createCourse(request: CourseCreateRequest): Observable<Course> {
    return this.http.post("/courses", request).map(response => response.json());
  }

  public updateCourse(id: number, request: CourseUpdateRequest): Observable<Course> {
    return this.http.put("/course/${id}", request).map(response => response.json());
  }

  public publishCourse(id: number, request: CoursePublishRequest): Observable<Course> {
    return this.http.put("/course/${id}/publish", request).map(response => response.json());
  }

  public tagCourse(id: number, request: TagCourseRequest): Observable<Course> {
    return this.http.put("/course/${id}/tag", request).map(response => response.json());
  }
